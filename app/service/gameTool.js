...
// 
const DdBjJndJg = (type, mode, number) => 
{
    ...
    for(let ii in _area)
    {
        ...
        {
            _num[ii][i] = number[_area[ii][i]];
            _sum[ii] = parseInt(_sum[ii]) + parseInt(number[_area[ii][i]]);
        }
    }
    _des.push(_num);
    _des.push(_sum);
...
const jndTopk = async(n) => 
{
    ...
    for (var i = 0, len = data.length; i < len; i += 6) 
    {
        if (i < 55)
        {
            const _one = data.slice(i, i + 6).join('');
            one.push(_one);
            const _two = parseInt(_one, 16);
            two.push(_two);
        }
    }
    let thr = {};
    ...
    const fou = Object.values(thr).sort((a, b) => { return b - a });
    const fiv = Object.keys(thr).sort((a, b) => { return thr[b] - thr[a] });
    return [
        n,
        data.join(''),
        one,
...
const PkJg = async(type, _d, peroids) => 
{
    const d = await jndTopk(_d);
    let _one;
    switch (type) {
        case '10':
